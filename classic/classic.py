...
# # Currency conversion - placeholder function (real: use API)
...
#     # STEP 1: Go to listings page
#     main_page.goto("https://www.classic.com/search?page=1&result_type=listings", timeout=60000)
...
#         try:
#             # This selects the gearbox <div> based on the path you provided
#             gearbox_element = listing.query_selector("div.flex.flex-wrap.justify-between.text-gray-500.table\\:justify-start.table\\:gap-x-3.table\\:gap-y-1 > div:nth-child(2)")
#             if gearbox_element:
...
#             "gearbox": og_gearbox
#         })
#     # Optional: limit to first N listings
...
#             # Description and Gearbox

#             # Auction House & Seller
#             desc_block = page.query_selector("div:has-text('Description')")
#             description = None
...
#                     seller_link_1 = history_blocks[1].query_selector("a")
#                     if seller_link_1:
#                         seller = (seller_link_1.inner_text()).strip()
#                 elif len(history_blocks) == 1:
#                     seller_link = history_blocks[0].query_selector("a")
#                     if seller_link:
#                         seller = (seller_link.inner_text()).strip()

#                 # Auction House (fallback)
#                 seller_tag = page.query_selector("a[href*='/dealer/']")
#                 auction_house = seller_tag.inner_text() if seller_tag else auction_house
...
#             # Country
#             location = page.query_selector("div:has-text(',')").inner_text()
#             country = location.split(",")[-1].strip()

#             # Spyder detection
#             spyder = any(keyword in model.lower() for keyword in ['spyder', 'spider'])
...
with sync_playwright() as p:
    ...
    main_page.goto("https://www.classic.com/search?page=10&result_type=listings", timeout=60000) # search query will be updated based on the make, model etc 
    main_page.wait_for_selector("#dealer-listings-table")
